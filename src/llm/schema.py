...
- Wage questions use `wages_job_bank` (normalized to HOURLY, annual/2080) and vacancy questions use `vacancies_statscan`; for both, filter `region = 'Toronto'` unless the question explicitly asks otherwise.
- `indeed_trends` is LONG-format: filter by `metric` in ('postings_index','wage_growth','ai_share'); `value` holds the number; `geography` is 'Toronto' (postings_index) or 'Canada' (wage_growth, ai_share).
...
- All data is already filtered to the Toronto / GTA market.
